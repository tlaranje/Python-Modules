...
class GardenManager:
    ...
    def add_plant(self, p):
        """Add a plant, raise PlantError if name is empty."""
        if not p.name:
    ...
    def water_plants(self, water):
        """Water plants, raise GardenError if insufficient water."""
    ...
    def check_plant_health(self):
        """Check plant health and raise PlantError for problems."""
        for p in self.plants:
            if p.water > 10:
                raise PlantError(
...
if __name__ == "__main__":
    ...
    try:
        ...
        garden.add_plant(Plant("Lettuce", 14, 13))
